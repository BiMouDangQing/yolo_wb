"""过暗处理模块：对过暗图片提亮、恢复暗部细节。

复用 tools/underexposure.py 中的算法，后台 QThread 处理，不阻塞界面。
"""
...
# 项目根/tools：QT/modules/underexposure.py -> parents[2] 即项目根目录
...
class UnderexposureWorker(QtCore.QThread):
    """后台执行过暗校正。"""
    ...
    def __init__(self, input_path, output_dir, strength, use_clahe,
    ...
    def run(self):
        mod = _load_tools_module("underexposure")
        correct = mod.correct_underexposure
        ...
        for i, src in enumerate(files, 1):
            ...
            out = correct(img, self.strength, self.use_clahe)
...
class UnderexposureModule(QtWidgets.QWidget):
    MODULE_TITLE = "过暗处理"
    ...
    def _build_ui(self):
        ...
        self.clahe_check = QtWidgets.QCheckBox("启用 CLAHE 增强暗部细节")
        self.clahe_check.setChecked(True)
        opt_row.addWidget(self.clahe_check)
        opt_row.addStretch()
        ...
        # 历史配置恢复
        self.history_bar = HistoryBar("underexposure", self._apply_config)
    ...
    def _restore_config(self):
        """恢复上次保存的路径与参数。"""
        self._apply_config(load_config("underexposure"))
    ...
    def _apply_config(self, cfg):
        ...
        self.clahe_check.setChecked(bool(cfg.get("use_clahe", True)))
        self.replace_check.setChecked(bool(cfg.get("replace", False)))

    def _persist_config(self):
        """保存当前路径与参数。"""
        save_config("underexposure", {
    ...
    def _start(self):
        ...
        self._worker = UnderexposureWorker(
